...
public class App {
	...
	static int training_set_size = 0;
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * quickly.
		 * 
		 * Epsilon = Greedy Policy to allow the agent to occasionally not to
		 * take the optimal action according to its experience.
		 * 
		 * Discount = Gamma, this models the fact that future rewards are worth
	...
	public static void train(SkynetAiAgent agent) {
		...
		while (agent.numTraining > 0) {
			...
			agent.info.add(player_hand_count + "#" + player_hand_value + "#" + dealer_hand_count + "#"
					+ dealer_hand_value + "#" + isWin + "#" + agent.getName());
		...
		if (!silent) {
			...
			System.out.println(
					"Agent specs: A:" + agent.getAlpha() + " D: " + agent.getDiscount() + " E: " + agent.epsilon);
	...
	public static void trainStockStuff(SkynetStockAgent agent, List<Stock> stockvalues) {
		...
		// Momentum kaava
		// Multiply that number by 100. M = (Price Today/Price Five Days Ago)
		// x100.
		...
		for (int i = 0; i < stockvalues.size(); i++) {
			...
			if (i == 0) {
				momentum = 0;
				stockvalues.get(i).setCommon_value(1);
			} else if (temp < 6) {
				momentum = (int) ((result / stockvalues.get(0).getCommon_value()) * 100);

			} else {
				momentum = (int) ((result / stockvalues.get(i - 5).getCommon_value()) * 100);
			}
		...
		training_set_size = stockvalues.size() + training_set_size;
		long end = System.currentTimeMillis();
		OpenFile.addLine("Training took " + df.format((end - start) / 1000d) + " seconds");
		OpenFile.addLine("Trainig set size " + training_set_size);
	...
	public static void tryWithMoneyStockStuff(SkynetStockAgent agent, List<Stock> stockvalues) {
		...
		for (int i = 0; i < stockvalues.size(); i++) {
			...
			if (action == 2 && temp > 5) { // Sell
				...
				if (momentum + 5 >= old_momemtum && stockvalues.get(i).getCommon_value() > old_common_value) {
					how_many_to_sell = (int) (agent.getStockCount() * 0.5);
					sell++;
					ai_action = "Sell";
				} else if (momentum + 3 >= old_momemtum && stockvalues.get(i).getCommon_value() > old_common_value) {
					how_many_to_sell = (int) (agent.getStockCount() * 0.2);
					sell++;
					ai_action = "Sell";
				}

				else if (stockvalues.get(i).getCommon_value() > old_common_value) {
			...
			if (action == 1 && temp > 5) {// buy stocks
				if (old_momemtum >= momentum - 7 && stockvalues.get(i).getCommon_value() < old_common_value) {
					usable_money = agent.getMoney() * 0.5;
					buy++;
					ai_action = "Buy";

				} else if (old_momemtum >= momentum - 5 && stockvalues.get(i).getCommon_value() < old_common_value) {
			...
			agent.info.add(
					old_price + "#" + old_pe_val + "#" + price + "#" + pe_val + "#" + isWin + "#" + agent.getName());
			double current_networth = agent.getMoney()
					+ (stockvalues.get(i).getAdjusted_price()) * agent.getStockCount();
			OpenFile.addQLine(Arrays.toString(newState), df.format(agent.getQval()), old_price, price, agent.getName(),
					momentum, ai_action, df.format(agent.getMoney()), agent.getStockCount(),
					df.format(current_networth), df.format(stockvalues.get(i).getCommon_value()));
		...
		double nw = agent.getMoney()
				+ (stockvalues.get(stockvalues.size() - 1).getAdjusted_price()) * agent.getStockCount();
		OpenFile.addLine("Agent Money: " + df.format(agent.getMoney()) + "€");
		OpenFile.addLine("Agent stock count: " + agent.getStockCount());
		OpenFile.addLine("Agent stock worth: "
				+ df.format((stockvalues.get(stockvalues.size() - 1).getAdjusted_price()) * agent.getStockCount())
				+ "€");
